      ...
      <div className="container mx-auto p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 justify-center items-center gap-4">
          {carts.map((cart, index) => (
            ...
            >
              <div className="absolute top-[-40px] right-[-20px] bg-primary-500 text-white rounded-full w-[80px] h-[80px] flex items-center justify-center">
              ...
              <div className="w-[95%] absolute left-[2.5%] right-[2.5%] top-[75%] h-[170px] bg-white p-4">
                <h3 className="text-secondary-500 text-lg font-bold">{cart.name}</h3>
                <p className="text-xl font-bold mb-2">{cart.title}</p>
                <span className="text-primary-500 line-through mr-2">$1500</span>
                <span className="text-primary-500 font-bold">${cart.price}</span>
              </div>
            </div>
          ))}
        </div>
